...
class _VowelsScreenState extends State<VowelsScreen> {
  ...
  int _duration(String word) {
    return (word.length * 350) + 600;
  }

  Future<void> _autoPlay(List list) async {
    ...
    for (int i = 0; i < list.length; i++) {
      ...
      await Future.delayed(
        Duration(milliseconds: _duration(list[i].symbol)),
      );
    }
  ...
  void _speakOne(String text, int index) async {
    _stop();
    ...
    _timer?.cancel();
    _timer = Timer(
      Duration(milliseconds: _duration(text)),
      () => setState(() => _currentIndex = -1),
    );
  }
  ...
  Widget build(BuildContext context) {
    final vowels = context.watch<NepaliVowelProvider>().vowel;

    return Scaffold(
      appBar: AppBar(
        title: Text(
          AppLocalizations.of(context)!.nepaliVowels,
        ...
        actions: [
          Padding(
            ...
            child: GestureDetector(
              onTap: () => _isPlaying ? _stop() : _autoPlay(vowels),
              child: Container(
            ...
            child: GridView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return AnimatedContainer(
                  ...
                  child: InkWell(
                    borderRadius: BorderRadius.circular(10),
                    onTap: () => _speakOne(item.symbol, index),
                    child: Center(
                      child: Column(
